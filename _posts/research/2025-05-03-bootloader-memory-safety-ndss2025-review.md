...
GRUB 최신 버전을 3주 동안 퍼징한 결과 14개의 취약점이 발견됐다. 이 중 일부는 이론적으로 Secure Boot 우회로 이어질 수 있다고 분석되었지만, 실제로 공격이 입증된 것은 아니기 때문에 가능성 수준으로 이해해야 한다.
...
NTFS 파일 시스템 파싱 로직에서 attribute 버퍼의 끝을 검증하지 않아 범위를 벗어난 메모리 접근이 발생한다. 조작된 NTFS 이미지로 힙을 덮어쓸 수 있으며, 논문은 이 취약점이 Secure Boot 우회로 이어질 가능성이 있다고 밝혔다.
...
네트워크 파일 시스템으로 심볼릭 링크 파일을 읽을 때 서버 응답 패킷의 경로 길이를 검증하지 않고 고정 길이(2048바이트) 전역 버퍼에 직접 복사한다. 네트워크 패킷의 길이 필드는 4바이트라 이론적으로 부트로더 전체 물리 메모리를 덮어쓸 수 있다.
...
논문에서는 부트로더 소스코드를 직접 수정해 퍼징을 위한 코드를 넣었기 때문에, 소스코드를 볼 수 없는 클로즈드 소스 부트로더에는 같은 방법을 그대로 적용하기 어렵다.

**Intel x86 아키텍처 종속**

kAFL이 Intel VT와 Intel-PT에 의존하기 때문에 다른 아키텍처에는 직접 적용 불가능하다. 논문에서도 파일 시스템이나 네트워크처럼 공통적인 부분은 비슷하지만, 디바이스 드라이버 같은 저수준 구현은 아키텍처마다 달라 적용이 어렵다고 설명했다.

**Exploitability 분석 부재**

발견된 취약점이 실제로 어떤 조건에서 Secure Boot 우회로 이어지는지 심층 분석은 다루지 않았다. 취약점 발견 자체에 집중한 논문이기에 실제로 공격에 사용 가능한 것이 얼마나 되는지는 아직 명확하지 않다.
...
**① FirmRCA (IEEE S&P 2025)** — arXiv 무료 공개 ([arxiv.org/abs/2410.18483](https://arxiv.org/abs/2410.18483))

이 논문이 "크래시를 찾는 것"에 집중했다면, FirmRCA는 "찾은 크래시가 왜 발생했는지"를 자동으로 추적한다.

**② SoK: Prudent Evaluation Practices for Fuzzing (IEEE S&P 2024)** — arXiv 무료 공개

퍼징 연구들이 실험 결과를 어떻게 평가하고 비교해야 하는지를 정리한 논문이다. 

**③ BootStomp (USENIX Security 2017)** — USENIX 무료 공개

이 논문이 한계로 지적한 기존 연구 중 하나다. taint analysis 기반으로 모바일 부트로더를 분석한 방식이 이 논문의 접근과 어떻게 다른지 비교해서 읽으면, 퍼징 vs 정적 분석의 차이를 볼 수 있을 거 같다.

---

### 퍼징 공부 계획

논문 프레임워크를 직접 실행하기에는 아직 환경 구성이나 퍼징 자체에 대한 이해가 부족하다고 느꼈다. 먼저 퍼징 기초를 익히고, 이후에 재현을 시도해볼 생각이다.

- [ ] **AFL++ 튜토리얼 따라하기** — [Fuzzing101](https://github.com/antonio-morales/Fuzzing101) Exercise 1~3
- [ ] **CVE-2022-2601 패치 전후 코드 읽기** — GRUB GitHub에서 diff 확인, 코드 수준으로 이해
