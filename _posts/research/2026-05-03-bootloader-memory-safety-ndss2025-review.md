...
부트로더는 OS가 시작되기 전, 하드웨어 위에서 직접 실행되며 모든 주변장치에 높은 수준의 접근 권한을 갖는다. 여기서 취약점이 터지면 Secure Boot 신뢰 체인 전체가 무너진다. 실제로 Intel, Acer, Lenovo 등 수백 개 모델에서 이미지 파싱 라이브러리 취약점으로 Secure Boot가 우회된 사례가 있으며, shim의 HTTP 구현 취약점으로 시스템 전체가 장악될 수 있었다.
...
구조적으로도 취약하다. 부트로더는 bare metal(OS 없이 하드웨어 위에서 직접 실행되는 환경)에서 동작하므로 표준 라이브러리나 OS의 도움 없이 모든 것을 자체 구현해야 한다. 파일 하나를 읽으려면 파일 경로 파싱, 파일 핸들러 관리, 파일 시스템, 블록 디바이스 접근, 스토리지 드라이버까지 전부 직접 구현해야 한다.
...
부트로더가 단순히 OS를 켜주는 프로그램이라고만 생각했는데, HTTP 다운로드나 이미지 파싱까지 지원한다는 걸 이 논문을 읽으면서 처음 알았다. 기능이 많다는 게 곧 공격당할 수 있는 곳도 많기에 구현 하나하나에서 보안을 생각해야 한다는 걸 다시 한번 느끼게 되었다.
...
도구를 먼저 만들고 적용한 게 아니라, 문제를 먼저 이해하고 도구를 맞춘 순서가 인상적이다. 

**둘째, 부트로더를 실제 환경 그대로 퍼징한다.** <br>
기존 퍼저(AFL·libFuzzer)를 부트로더에 적용하면 두 가지 이유로 실패한다. 
부트로더는 bare-metal 환경에서 동작하기 때문에, AFL이나 libFuzzer 같은 퍼징 도구를 직접 적용하기 어렵고, 기존 sanitizer(메모리 오류를 런터임에 감지하는 도구)도 호환되지 않는다. 또한 부트로더를 일반 프로그램처럼 변환해 퍼징을 수행하면 실제 실행 환경과 달라서, 발견된 오류가 실제에서도 재현되지 않을 수 있다.
...
이러한 문제를 해결하기 위해, Hypervisor(가상 머신을 실행·관리하는 소프트웨어 계층) 위에서 가상 머신을 실행해 부트로더가 실제와 유사한 환경에서 동작하도록 만든 뒤 퍼징을 수행하는 방식을 사용한다.

hypervisor라는 개념을 이 논문을 통해 처음 접했다. 가상 머신을 실행·관리하는 소프트웨어 계층이라는 설명을 읽고, 그 위에서 부트로더를 실제 환경처럼 실행할 수 있다는 것도 처음 알게 됐다. 퍼징 환경을 맞추기 위해 환경 자체를 만들어버린다는 발상이 흥미로웠다.
...
3. **Intel-PT가 부트로더의 코드 실행 경로를 추적한다. (Coverage)** 
   Intel-PT(CPU가 실행한 코드 경로를 하드웨어 수준에서 추적하는 기능)로 어떤 코드가 더 실행됐는지 기록한다. 아직 실행되지 않은 경로가 있으면 그쪽으로 입력을 유도해 더 많은 코드를 탐색한다.
...
harness(퍼저가 타깃 함수를 호출할 수 있도록 작성하는 래퍼 코드)는 소스코드에 직접 삽입하는 방식으로 작성했다.
...
힙 오버플로우(heap overflow)는 할당된 버퍼 경계를 넘어 인접 메모리를 덮어쓰는 취약점을 말한다. barebox의 ARP 구현에서 발견된 취약점이다. 수신된 패킷을 고정 크기(PKTSIZE = 1536바이트) 버퍼에 복사하는데, 점보 프레임처럼 패킷이 더 클 경우 `memcpy`가 버퍼 경계를 넘어 인접 메모리를 덮어쓴다. 입력 크기를 검증하지 않은 전형적인 힙 오버플로우 패턴이다.

PKTSIZE를 1536으로 고정해놓고 들어오는 패킷 크기를 안 봤다는 게, 생각보다 단순한 실수에서 취약점이 생긴다는 걸 보여주는 것 같았다.
...
EXT 파일 시스템 슈퍼블록에서 읽어온 `inodes_per_group` 값을 검증 없이 그대로 나눗셈에 사용한다. 공격자가 이 값을 0으로 조작한 파일 시스템 이미지를 USB에 담아 꽂으면 divide by zero(어떤 숫자를 0으로 나누려 할 떄 발생하는 오류)가 발생한다. Listing 1과 달리 네트워크가 아닌 스토리지 입력 경로의 취약점이다.

파일 시스템 이미지를 조작해서 USB에 담아 꽂으면 된다는 게 생각보다 공격 진입장벽이 낮다는 느낌이 들었다.
...
```
 ...
 BMP 이미지 디코더에서 발견된 취약점으로, integer underflow(부호 없는 정수에서 0보다 작은 값이 되면 매우 큰 값으로 바뀌는 현상)로 인한 힙 오버플로우다. `RealPixelWidth`가 2보다 작을 경우 `RealPixelWidth - 2`는 unsigned integer 언더플로우로 인해 매우 큰 값이 되고, 루프가 대량의 메모리를 덮어쓴다. 입력값의 범위를 검증하지 않아 생긴 정수 오버플로우가 힙 오버플로우로 이어진 사례다. 부트로더가 이미지 파일을 파싱한다는 사실 자체가 공격 표면이 된다는 걸 보여준다.
...
| CVE | 대상 | 경로 | 유형 | 핵심 |
|---|---|---|---|---|
| CVE-2023-4692/4693 | GRUB | 스토리지 | 힙 오버플로우 | NTFS attribute 버퍼 끝 미검증. Secure Boot 우회 가능성 |
| CVE-2020-8432 | Das U-Boot | 콘솔 | double-free | gpt rename 실패 시 버퍼 이중 해제 |
| CVE-2022-33103 | Das U-Boot | 스토리지 | 버퍼 오버플로우 | squashfs 파일명 길이 미검증 |
| CVE-2019-15937/38 | barebox | 네트워크 | 버퍼 오버플로우 | NFS 응답 패킷 길이 미검증, 이론상 전체 메모리 덮어쓰기 가능 |
| CVE-2023-40547 | shim | 네트워크 | 힙 오버플로우 | HTTP content-length 미검증, sanitizer로만 탐지 |

- double-free: 이미 해제(free)한 메모리를 한 번 더 해제하는 취약점. 힙 메모리 구조를 손상시켜 공격자가 임의 코드를 실행할 수 있는 조건을 만들 수 있다.

5개 CVE 모두 공격 표면 분류(스토리지 48%, 네트워크 28%, 콘솔 14%)와 일치하는 경로에서 발견됐다. 논문 앞부분의 CVE 분석이 실제 발견으로 그대로 이어진 셈이다. 특히 CVE-2023-40547은 힙 오버플로우가 직접 크래시를 내지 않아 커스텀 sanitizer가 없었다면 놓쳤을 취약점이다. 도구 설계의 중요성을 보여주는 사례라고 느꼈다.
...
취약점을 찾는 것과 그것을 실제 공격으로 연결하는 것은 전혀 다른 문제라는 게 와닿았다. 38개를 발견했어도 그중 몇 개가 실제로 쓰일 수 있는지 모른다는 점이, 보안 연구가 취약점 발견에서 끝나지 않는다는 걸 보여주는 것 같았다.
...
이 논문이 "크래시를 찾는 것"에 집중했다면, FirmRCA는 "찾은 크래시가 왜 발생했는지"를 자동으로 추적한다. 퍼징에서 원인 분석으로 이어지는 흐름으로 읽으면 좋을 거 같다.
